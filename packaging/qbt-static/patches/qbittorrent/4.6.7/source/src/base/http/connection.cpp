...
void Connection::sendResponse(const Response &response) const
{
    ...
    m_socket->write(responseHeadersToByteArray(preparedResponse));
    if (!preparedResponse.content.isEmpty())
        m_socket->write(preparedResponse.content);
}
...
bool Connection::acceptsGzipEncoding(QString codings)
{
    ...
    const auto hasPrefix = [](const QString &value, const QStringView prefix) -> bool
    {
    ...
    const auto isCodingAvailable = [&hasPrefix](const QStringList &list, const QStringView encoding) -> bool
    {
        for (const QString &str : list)
        {
            if (!hasPrefix(str, encoding))
                continue;

            // without quality values
            if (str.size() == encoding.size())
                return true;

            const qsizetype suffixPos = encoding.size();
            const QStringView suffix = QStringView {str}.mid(suffixPos);
            if ((suffix.size() < 3)
                || (suffix.at(0) != u';')
                || (suffix.at(1) != u'q')
                || (suffix.at(2) != u'='))
            ...
            bool ok = false;
            const double qvalue = str.mid(suffixPos + 3).toDouble(&ok);
            if (!ok || (qvalue <= 0))
                return false;
