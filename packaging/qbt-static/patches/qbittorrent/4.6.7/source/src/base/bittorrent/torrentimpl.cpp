...
bool TorrentImpl::shouldDeferFilePaths() const
{
    return Preferences::instance()->isExperimentalDeferredTorrentFilePathsEnabled();
}

bool TorrentImpl::shouldDeferFileState() const
{
    return Preferences::instance()->isExperimentalDeferredTorrentFileStateEnabled();
}

bool TorrentImpl::shouldUseSparseRenamedFiles() const
{
    return Preferences::instance()->isExperimentalSparseTorrentRenamedFilesEnabled();
}

Path TorrentImpl::resolvedFilePath(const int index) const
{
    return resolvedLogicalFilePath(m_torrentInfo, m_contentLayout, m_ltAddTorrentParams.renamed_files, index);
}

PathList TorrentImpl::resolvedFilePaths() const
{
    PathList paths;
    const int count = m_torrentInfo.filesCount();
    paths.reserve(count);
    for (int i = 0; i < count; ++i)
        paths.append(resolvedFilePath(i));

    return paths;
}

int TorrentImpl::resolvedFileIndex(const lt::file_index_t nativeIndex) const
{
    if (!shouldDeferFileState())
        return m_indexMap.value(nativeIndex, -1);

    return m_torrentInfo.nativeIndexes().indexOf(nativeIndex);
}

QVector<DownloadPriority> TorrentImpl::resolvedFilePriorities() const
{
    if (!shouldDeferFileState())
        return m_filePriorities;

    QVector<DownloadPriority> priorities;
    if (!hasMetadata())
        return priorities;

    const auto nativeIndexes = m_torrentInfo.nativeIndexes();
    const std::vector<lt::download_priority_t> nativePriorities =
            resized(m_ltAddTorrentParams.file_priorities, m_torrentInfo.nativeInfo()->files().num_files()
                    , LT::toNative(m_ltAddTorrentParams.file_priorities.empty() ? DownloadPriority::Normal : DownloadPriority::Ignored));

    priorities.reserve(nativeIndexes.size());
    for (const lt::file_index_t nativeIndex : nativeIndexes)
        priorities.append(LT::fromNative(nativePriorities[LT::toUnderlyingType(nativeIndex)]));

    return priorities;
}

bool TorrentImpl::isCompletedFile(const int index) const
{
    Q_ASSERT(index >= 0);
    Q_ASSERT(index < filesCount());
    if (Q_UNLIKELY((index < 0) || (index >= filesCount())))
        return false;

    if (!shouldDeferFileState())
        return m_completedFiles.at(index);

    if (!hasMetadata())
        return false;

    const qlonglong size = fileSize(index);
    if (size <= 0)
        return true;

    const QBitArray pieces = (m_pieces.isEmpty() ? LT::toQBitArray(m_nativeStatus.pieces) : m_pieces);
    ...
    {
        if ((pieceIndex >= pieces.size()) || !pieces.testBit(pieceIndex))
            return false;
    }
...
QVector<std::int64_t> TorrentImpl::resolvedFilesProgress() const
{
    if (!shouldDeferFileState())
        return m_filesProgress;

    QVector<std::int64_t> progress;
    if (!hasMetadata())
        return progress;

    const int count = filesCount();
    progress.fill(0, count);

    const QBitArray pieces = (m_pieces.isEmpty() ? LT::toQBitArray(m_nativeStatus.pieces) : m_pieces);
    const int64_t pieceSize = m_torrentInfo.pieceLength();
    for (int pieceIndex = 0; pieceIndex < pieces.size(); ++pieceIndex)
    {
        if (!pieces.at(pieceIndex))
            continue;

        int64_t size = m_torrentInfo.pieceLength(pieceIndex);
        int64_t pieceOffset = static_cast<int64_t>(pieceIndex) * pieceSize;
        for (const int fileIndex : asConst(m_torrentInfo.fileIndicesForPiece(pieceIndex)))
        {
...
void TorrentImpl::clearCachedFileState()
{
    ...
    if (!m_filesProgress.isEmpty())
        m_filesProgress.clear();

    refreshResidentMemberFamilyBytes();
}

void TorrentImpl::refreshResidentMemberFamilyBytes()
{
    ResidentMemberFamilyBytes bytes;
    const auto estimateQStringPayloadBytes = [](const QString &string) -> qint64
    {
        return static_cast<qint64>(string.size()) * static_cast<qint64>(sizeof(QChar));
    };

    for (const Path &filePath : asConst(m_filePaths))
    {
        const QString pathString = filePath.toString();
        bytes.filePaths += static_cast<qint64>(sizeof(Path));
        bytes.filePaths += static_cast<qint64>(pathString.size() * sizeof(QChar));
    }

    constexpr qint64 hashNodeOverhead = static_cast<qint64>(sizeof(void *) * 4);
    constexpr qint64 mapNodeOverhead = static_cast<qint64>(sizeof(void *) * 4);
    const auto estimateBitfieldBytes = [](const auto &bitfield) -> qint64
    {
        if (bitfield.empty())
            return 0;
        return static_cast<qint64>(sizeof(std::uint32_t))
            + (static_cast<qint64>(bitfield.num_words()) * static_cast<qint64>(sizeof(std::uint32_t)));
    };

    bytes.indexMap = static_cast<qint64>(m_indexMap.size())
        * (static_cast<qint64>(sizeof(lt::file_index_t))
            + static_cast<qint64>(sizeof(int))
            + hashNodeOverhead);
    bytes.filePriorities = static_cast<qint64>(m_filePriorities.size())
        * static_cast<qint64>(sizeof(DownloadPriority));
    bytes.completedFiles = static_cast<qint64>((m_completedFiles.size() + 7) / 8);
    bytes.filesProgress = static_cast<qint64>(m_filesProgress.size())
        * static_cast<qint64>(sizeof(std::int64_t));
    bytes.piecesBitfield = static_cast<qint64>((m_torrentInfo.piecesCount() + 7) / 8);
    bytes.atpFilePriorities = static_cast<qint64>(m_ltAddTorrentParams.file_priorities.size())
        * static_cast<qint64>(sizeof(lt::download_priority_t));
    bytes.atpTrackerTiers = static_cast<qint64>(m_ltAddTorrentParams.tracker_tiers.size())
        * static_cast<qint64>(sizeof(int));

    for (const std::string &tracker : m_ltAddTorrentParams.trackers)
        bytes.atpTrackers += static_cast<qint64>(sizeof(std::string) + tracker.size());
    for (const std::string &urlSeed : m_ltAddTorrentParams.url_seeds)
        bytes.atpUrlSeeds += static_cast<qint64>(sizeof(std::string) + urlSeed.size());
    for (const auto &renamedFile : m_ltAddTorrentParams.renamed_files)
    {
        bytes.atpRenamedFiles += static_cast<qint64>(sizeof(lt::file_index_t));
        bytes.atpRenamedFiles += static_cast<qint64>(sizeof(std::string) + renamedFile.second.size());
        bytes.atpRenamedFiles += mapNodeOverhead;
    }
    for (const auto &unfinishedPiece : m_ltAddTorrentParams.unfinished_pieces)
    {
        bytes.atpUnfinishedPieces += static_cast<qint64>(sizeof(lt::piece_index_t));
        bytes.atpUnfinishedPieces += estimateBitfieldBytes(unfinishedPiece.second);
        bytes.atpUnfinishedPieces += mapNodeOverhead;
    }
    bytes.atpHavePieces = estimateBitfieldBytes(m_ltAddTorrentParams.have_pieces);
    bytes.atpVerifiedPieces = estimateBitfieldBytes(m_ltAddTorrentParams.verified_pieces);
    bytes.atpSavePath = static_cast<qint64>(m_ltAddTorrentParams.save_path.size());
    bytes.persistentName = estimateQStringPayloadBytes(m_name);
    bytes.persistentSavePath = estimateQStringPayloadBytes(m_savePath.toString());
    bytes.persistentDownloadPath = estimateQStringPayloadBytes(m_downloadPath.toString());
    bytes.persistentCategory = estimateQStringPayloadBytes(m_category);
    bytes.trackerEntries = static_cast<qint64>(m_trackerEntries.capacity())
        * static_cast<qint64>(sizeof(TrackerEntry));
    for (const TrackerEntry &entry : asConst(m_trackerEntries))
        bytes.trackerEntries += estimateQStringPayloadBytes(entry.url);
    bytes.urlSeeds = static_cast<qint64>(m_urlSeeds.capacity())
        * static_cast<qint64>(sizeof(QUrl));
    for (const QUrl &urlSeed : asConst(m_urlSeeds))
        bytes.urlSeeds += estimateQStringPayloadBytes(urlSeed.toString());
    bytes.nativeStatusCoreStrings = static_cast<qint64>(m_nativeStatus.name.size())
        + static_cast<qint64>(m_nativeStatus.save_path.size())
        + static_cast<qint64>(m_nativeStatus.current_tracker.size());
    bytes.nativeStatusPiecesBitfield = estimateBitfieldBytes(m_nativeStatus.pieces);
    const std::shared_ptr<const lt::torrent_info> nativeInfo = m_torrentInfo.nativeInfoShared();
    const std::shared_ptr<const lt::torrent_info> runtimeNativeInfo = nativeTorrentInfo();
    if (nativeInfo)
    {
#ifdef QBT_USES_LIBTORRENT2
        bytes.torrentInfoMetadata = static_cast<qint64>(nativeInfo->info_section().size());
        const bool metadataAliasedWithRuntime = (runtimeNativeInfo
            && (nativeInfo->info_section().data() == runtimeNativeInfo->info_section().data())
            && (nativeInfo->info_section().size() == runtimeNativeInfo->info_section().size()));
#else
        bytes.torrentInfoMetadata = static_cast<qint64>(nativeInfo->metadata_size());
        const bool metadataAliasedWithRuntime = (runtimeNativeInfo
            && (nativeInfo->metadata_size() == runtimeNativeInfo->metadata_size())
            && (nativeInfo->metadata().get() == runtimeNativeInfo->metadata().get()));
#endif
        if (metadataAliasedWithRuntime)
            bytes.torrentInfoMetadataAliased = bytes.torrentInfoMetadata;
        else
            bytes.torrentInfoMetadataUnique = bytes.torrentInfoMetadata;
    }

    if (m_ltAddTorrentParams.ti)
    {
        const std::shared_ptr<const lt::torrent_info> atpInfo
            = std::const_pointer_cast<const lt::torrent_info>(m_ltAddTorrentParams.ti);
#ifdef QBT_USES_LIBTORRENT2
        bytes.atpTorrentInfoMetadata = static_cast<qint64>(atpInfo->info_section().size());
#else
        bytes.atpTorrentInfoMetadata = static_cast<qint64>(atpInfo->metadata_size());
#endif
        if (nativeInfo && (nativeInfo.get() == atpInfo.get()))
            bytes.atpTorrentInfoMetadataAliased = bytes.atpTorrentInfoMetadata;
        else
            bytes.atpTorrentInfoMetadataUnique = bytes.atpTorrentInfoMetadata;
    }

    m_residentMemberFamilyBytes = bytes;
}

void TorrentImpl::appendInitialFilePath(const Path &filePath)
{
    if (!shouldDeferFilePaths())
        m_filePaths.append(filePath);
}

void TorrentImpl::clearCachedFilePaths()
{
...
void TorrentImpl::updateCachedFilePath(const int index, const Path &filePath)
{
    if (!shouldDeferFilePaths())
    {
    ...
    , m_uploadLimit(cleanLimitValue(m_ltAddTorrentParams.upload_limit))
{
    if (m_ltAddTorrentParams.ti)
    {
        ...
        if (shouldUseSparseRenamedFiles())
        {
            m_ltAddTorrentParams.renamed_files = storedRenamedFiles(m_torrentInfo, m_contentLayout
                    , mappedFilePaths(m_torrentInfo, m_contentLayout, m_ltAddTorrentParams.renamed_files), true);
        }
        ...
        for (int i = 0; i < filesCount; ++i)
        {
            ...
            appendInitialFilePath(resolvedFilePath(i));
...
Path TorrentImpl::wantedActualPath(int index, const Path &path) const
{
    if (m_session->isAppendExtensionEnabled()
            && (fileSize(index) > 0) && !isCompletedFile(index))
    {
        return path + QB_EXT;
    }
                           ...
                           , thisTorrent = QPointer<TorrentImpl>(this)]
    {
        try
        {
            ...
            session->invoke([session, thisTorrent, currentSeeds, addedUrlSeeds]
            {
                ...
                thisTorrent->refreshResidentMemberFamilyBytes();
...
ResidentMemberFamilyBytes TorrentImpl::residentMemberFamilyBytes() const
{
    return m_residentMemberFamilyBytes;
}

QVector<DownloadPriority> TorrentImpl::filePriorities() const
{
    return resolvedFilePriorities();
}
...
QVector<qreal> TorrentImpl::filesProgress() const
{
    ...
    const QVector<std::int64_t> progress = resolvedFilesProgress();
    const int count = progress.size();
    ...
    for (int i = 0; i < count; ++i)
    {
        if (isCompletedFile(i))
        {
            result << 1;
            continue;
        }

        const int64_t fileProgress = progress.at(i);
        const int64_t size = fileSize(i);
        if ((size <= 0) || (fileProgress == size))
            result << 1;
        else
            result << (fileProgress / static_cast<qreal>(size));
    }
...
void TorrentImpl::applyFirstLastPiecePriority(const bool enabled)
{
    ...
    const QVector<DownloadPriority> priorities = filePriorities();
    for (int fileIndex = 0; fileIndex < priorities.size(); ++fileIndex)
    {
        const DownloadPriority filePrio = priorities[fileIndex];
...
void TorrentImpl::endReceivedMetadataHandling(const Path &savePath, const PathList &fileNames)
{
    ...
    for (int i = 0; i < fileNames.size(); ++i)
    {
        ...
        updateStoredRenamedFile(m_torrentInfo, m_contentLayout, p.renamed_files, i, actualFilePath
                , shouldUseSparseRenamedFiles());

        const Path filePath = actualFilePath.removedExtension(QB_EXT);
        appendInitialFilePath(filePath);
    ...
    refreshResidentMemberFamilyBytes();
    if (shouldUseSparseRenamedFiles())
        p.renamed_files = storedRenamedFiles(m_torrentInfo, m_contentLayout, fileNames, true);
...
try
{
    ...
    if (!p.ti && hasMetadata())
    {
        // Keep ATP metadata sparse in resident state, but provide libtorrent
        // with metadata at the add boundary when needed.
...
void TorrentImpl::handleSaveResumeDataAlert(const lt::save_resume_data_alert *p)
{
    ...
    if (m_maintenanceJob == MaintenanceJob::HandleMetadata)
    {
        ...
        PathList filePaths = mappedFilePaths(metadata, m_contentLayout, renamedFiles);
...
{
    const int fileIndex = resolvedFileIndex(p->index);
    Q_ASSERT(fileIndex >= 0);

    // Remove empty leftover folders
    // For example renaming "a/b/c" to "d/b/c", then folders "a/b" and "a" will
    // be removed if they are empty
    const Path oldFilePath = filePath(fileIndex);
    ...
    {
        updateStoredRenamedFile(m_torrentInfo, m_contentLayout, m_ltAddTorrentParams.renamed_files, fileIndex
                , newFilePath, shouldUseSparseRenamedFiles());
        updateCachedFilePath(fileIndex, newFilePath);
