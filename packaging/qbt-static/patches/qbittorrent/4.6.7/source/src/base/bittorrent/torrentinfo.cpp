...
namespace
{
    QVector<lt::file_index_t> nativeIndexesFor(const std::shared_ptr<const lt::torrent_info> &nativeInfo)
    {
        QVector<lt::file_index_t> nativeIndexes;
        Q_ASSERT(nativeInfo && nativeInfo->is_valid() && (nativeInfo->num_files() > 0));
        if (!nativeInfo || !nativeInfo->is_valid() || (nativeInfo->num_files() <= 0))
            return nativeIndexes;

        const lt::file_storage &fileStorage = nativeInfo->orig_files();
        nativeIndexes.reserve(fileStorage.num_files());
        for (const lt::file_index_t nativeIndex : fileStorage.file_range())
        {
            if (!fileStorage.pad_file_at(nativeIndex))
                nativeIndexes.append(nativeIndex);
        }
...
TorrentInfo::TorrentInfo(const lt::torrent_info &nativeInfo)
    : TorrentInfo(std::make_shared<const lt::torrent_info>(nativeInfo))
{
}

TorrentInfo::TorrentInfo(std::shared_ptr<const lt::torrent_info> nativeInfo)
    : m_nativeInfo {std::move(nativeInfo)}
{
    m_nativeIndexes = nativeIndexesFor(m_nativeInfo);
}
...
std::shared_ptr<const lt::torrent_info> TorrentInfo::nativeInfoShared() const
{
    return m_nativeInfo;
}
