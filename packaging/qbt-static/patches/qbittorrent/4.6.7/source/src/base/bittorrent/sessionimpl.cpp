...
namespace
{
    ...
    lt::add_torrent_params expandedAddTorrentParams(const LoadTorrentParams &params)
    {
        lt::add_torrent_params addTorrentParams = params.ltAddTorrentParams;
        if (Preferences::instance()->isExperimentalSparseTorrentRenamedFilesEnabled()
                && addTorrentParams.ti && addTorrentParams.ti->is_valid())
        {
            addTorrentParams.renamed_files = expandedRenamedFiles(TorrentInfo(*addTorrentParams.ti)
                    , params.contentLayout, addTorrentParams.renamed_files);
        }
...
#endif
    m_nativeSession->async_add_torrent(expandedAddTorrentParams(resumeData));
...
lt::settings_pack SessionImpl::loadLTSettings() const
{
    ...
    settingsPack.set_int(lt::settings_pack::alert_queue_size, Preferences::instance()->sessionAlertQueueSize());
...
void SessionImpl::fileSearchFinished(const TorrentID &id, const Path &savePath, const PathList &fileNames)
{
    ...
    if (loadingTorrentsIter != m_loadingTorrents.end())
    {
        ...
        p.renamed_files = storedRenamedFiles(torrentInfo, params.contentLayout, fileNames
                , Preferences::instance()->isExperimentalSparseTorrentRenamedFilesEnabled());

        m_nativeSession->async_add_torrent(expandedAddTorrentParams(params));
    }
}
...
bool SessionImpl::addTorrent_impl(const std::variant<MagnetUri, TorrentInfo> &source, const AddTorrentParams &addTorrentParams)
{
    ...
    if (hasMetadata)
    {
        ...
        if (filePaths.isEmpty())
            filePaths = defaultLogicalFilePaths(torrentInfo, loadTorrentParams.contentLayout);
        ...
        if (!isFindingIncompleteFiles)
            p.renamed_files = storedRenamedFiles(torrentInfo, loadTorrentParams.contentLayout, filePaths
                    , Preferences::instance()->isExperimentalSparseTorrentRenamedFilesEnabled());
    ...
    if (!isFindingIncompleteFiles)
        m_nativeSession->async_add_torrent(expandedAddTorrentParams(loadTorrentParams));
        ...
        , const Path &downloadPath, const PathList &filePaths) const
{
    ...
    const PathList originalFileNames = (filePaths.isEmpty()
            ? defaultLogicalFilePaths(torrentInfo, TorrentContentLayout::Original)
            : filePaths);
