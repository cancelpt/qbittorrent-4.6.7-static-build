...
#include "api/maindatasyncstore.h"
...
WebApplication::WebApplication(IApplication *app, QObject *parent)
    ...
    , m_maindataSyncStore {new WebUISync::MaindataSyncStore(this)}
{
    ...
    connect(m_freeDiskSpaceChecker, &FreeDiskSpaceChecker::checked
            , m_maindataSyncStore, &WebUISync::MaindataSyncStore::updateFreeDiskSpace);
...
void WebApplication::doProcessRequest()
{
    ...
    if (!controller)
    {
        if ((scope == u"sync") && session())
        {
            m_maindataSyncStore->updateFreeDiskSpace(m_freeDiskSpaceChecker->lastResult());
            controller = session()->registerAPIController<SyncController>(u"sync"_s, m_maindataSyncStore);
            m_maindataSyncStore->recordWebSessionEvent(u"web_session_sync_controller_created"_s, {
                {u"client_id"_s, clientId()},
                {u"request_path"_s, request().path},
                {u"request_method"_s, request().method},
                {u"user_agent_summary"_s, WebUITrace::summarizeUserAgentForTrace(request().headers.value(u"user-agent"_s))},
                {u"user_agent_fingerprint"_s, WebUITrace::makeTraceFingerprint(request().headers.value(u"user-agent"_s))},
                {u"session_fingerprint"_s, WebUITrace::makeTraceFingerprint(session()->id())},
            });
        }
        else if (scope == u"auth")
    ...
    try
    {
        ...
        if (result.userType() == qMetaTypeId<RawAPIResult>())
        {
            const RawAPIResult rawResult = result.value<RawAPIResult>();
            print(rawResult.data, rawResult.contentType);
        }
        else switch (result.userType())
        {
    ...
    if (!sessionId.isEmpty())
    {
        ...
        if (m_currentSession)
        {
            if (m_currentSession->hasExpired(m_sessionTimeout))
            {
                ...
                const QString expiredSessionId = m_currentSession->id();
                const qint64 expiredSessionIdleMs = m_currentSession->idleMs();
                delete m_sessions.take(sessionId);
                m_currentSession = nullptr;
                m_maindataSyncStore->recordWebSessionEvent(u"web_session_ended"_s
                    , WebUITrace::buildSessionEndTraceDetails(u"expired_lookup"_s, clientId(), request().path
                        , request().method, request().headers.value(u"user-agent"_s), expiredSessionId
                        , m_sessions.size(), expiredSessionIdleMs));
                sessionStartReason = u"auth_bypass_sid_expired"_s;
            }
            else
            {
                m_currentSession->updateTimestamp();
                m_maindataSyncStore->recordWebSessionEvent(u"web_session_reused"_s
                    , WebUITrace::buildSessionStartTraceDetails(u"existing_session_hit"_s, clientId()
                        , request().path, request().method, request().headers.value(u"user-agent"_s)
                        , isAuthNeeded(), true, sessionId, {}, m_sessions.size(), m_sessionTimeout, 0));
            }
        }
        else
        {
            qDebug() << Q_FUNC_INFO << "session does not exist!";
            sessionStartReason = u"auth_bypass_sid_lookup_miss"_s;
        }
    }

    if (!m_currentSession && !isAuthNeeded())
    {
        if (sessionStartReason.isEmpty())
            sessionStartReason = u"auth_bypass_no_cookie"_s;
        sessionStartWithTrace(sessionStartReason, !sessionId.isEmpty(), sessionId);
    }
}
...
{
    sessionStartWithTrace(u"explicit_session_start"_s, false, {});
}

void WebApplication::sessionStartWithTrace(const QString &reason, const bool sessionCookiePresent
    , const QString &presentedSessionId)
{
    Q_ASSERT(!m_currentSession);

    // remove outdated sessions
    qsizetype expiredSessionsSwept = 0;
    Algorithm::removeIf(m_sessions, [this, &expiredSessionsSwept](const QString &, const WebSession *session)
    {
        if (session->hasExpired(m_sessionTimeout))
        {
            delete session;
            ++expiredSessionsSwept;
            return true;
        }
    ...
    m_maindataSyncStore->recordWebSessionEvent(u"web_session_started"_s
        , WebUITrace::buildSessionStartTraceDetails(reason, clientId(), request().path, request().method
            , request().headers.value(u"user-agent"_s), isAuthNeeded(), sessionCookiePresent
            , presentedSessionId, m_currentSession->id(), m_sessions.size(), m_sessionTimeout
            , expiredSessionsSwept));
}
...
void WebApplication::sessionEnd()
{
    ...
    const QString endedSessionId = m_currentSession->id();
    const qint64 sessionIdleMs = m_currentSession->idleMs();
    delete m_sessions.take(m_currentSession->id());
    m_currentSession = nullptr;

    setHeader({Http::HEADER_SET_COOKIE, QString::fromLatin1(cookie.toRawForm())});
    m_maindataSyncStore->recordWebSessionEvent(u"web_session_ended"_s
        , WebUITrace::buildSessionEndTraceDetails(u"explicit_end"_s, clientId(), request().path
            , request().method, request().headers.value(u"user-agent"_s), endedSessionId
            , m_sessions.size(), sessionIdleMs));
}
...
qint64 WebSession::idleMs() const
{
    if (!m_timer.isValid())
        return 0;
